...
def encender_equipo(n):
    if n == "":
        pass #Solo una sintaxis de integridad de la informacion llamada desde labview. Yo debo enviar una cadena vacia
    import serial
    ser = serial.Serial(
        port="COM19",  # numCOM es un valor que se ingresa manualmente. Asi podemos seleccionar dinamicamente el COM
        baudrate=9600,  # Velocidad de transmisión
        timeout=1  # Tiempo de espera para lecturas
    )
    add = 0
    load = Load(add, ser)
...
def set_load(carga):
    ...
    load = Load(add, ser)
    load.remote_control = True
    load.set_load_resistance(carga)
    time.sleep(0.5)
...
def set_max_current(cur):
    ...
    load.remote_control = True
    cur = float(cur)
    load.max_current = cur
    load.remote_control = False
    ser.close()
    return "OK"

def set_max_power(pw):
    import serial
...
def read_current(n):
    ...
    :param n: Simplemente se utiliza para integridad de la funcion desde labview
    :return: Se devuelve la lectura de la corriente. Empieza a perder precision al tercer digito (una lectura de 101 mA puede ser facilmente 104mA)
    """
    if n=="":
        pass
    import serial
...
def set_current(corriente):
    ...
    load = Load(add, ser)
    load.remote_control = True
    load.set_load_current(corriente)
    time.sleep(0.5)
